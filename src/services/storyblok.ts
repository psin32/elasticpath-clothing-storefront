...
export const getCategoryContent = async (categories: string[], apiKey?: string) => {
    const headerResponse = await fetch(`https://api-us.storyblok.com/v2/cdn/stories/categories/${categories.join("/")}?token=${apiKey? apiKey : process.env.NEXT_PUBLIC_STORYBLOK_API_KEY}&version=${version}`, { next: { revalidate: 10 } });
    return await headerResponse.json();
}

export const getContentByFolder = async (type: string, page: string[], apiKey?: string) => {
    const headerResponse = await fetch(`https://api-us.storyblok.com/v2/cdn/stories/${type}/${page.join("/")}?token=${apiKey? apiKey : process.env.NEXT_PUBLIC_STORYBLOK_API_KEY}&version=${version}`, { next: { revalidate: 10 } });
    return await headerResponse.json();
}

export const getProductContent = async (productId: string, locale?: string, apiKey?: string) => {
    const headerResponse = await fetch(`https://api-us.storyblok.com/v2/cdn/stories/products/${productId}?token=${apiKey? apiKey : process.env.NEXT_PUBLIC_STORYBLOK_API_KEY}&version=${version}&language=${locale}`, { next: { revalidate: 10 } });
    return await headerResponse.json();
}
